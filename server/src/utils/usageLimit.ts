...
export async function checkUsageLimit(
  ip: string,
  userId?: number
): Promise<{ canUse: boolean; remaining: number; isLoggedIn: boolean; used: number }> {
  if (userId) {
    return { canUse: true, remaining: Infinity, isLoggedIn: true, used: 0 };
  }
  ...
  if (!record || now > record.resetTime) {
    ...
    ipUsageCache.set(ip, record);
  }

  const remaining = FREE_LIMIT - record.count;
  const canUse = remaining > 0;

  return { canUse, remaining, isLoggedIn: false, used: record.count };
}
...
export async function recordUsage(ip: string, userId?: number): Promise<void> {
  ...
  if (record) {
    ...
    ipUsageCache.set(ip, { count: 1, resetTime: Date.now() + 86400000 });
  }
}